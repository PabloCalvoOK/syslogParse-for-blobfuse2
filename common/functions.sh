...
prebuild() {
  ...
  backupCommand=$(jq -r ".\"$dockerType\".backupCommand" $output_file)
  restoreCommand=$(jq -r ".\"$dockerType\".restoreCommand" $output_file)
  ...
  if [[ $dockerType == "minio" ]]; then
    if [ -d files/$dockerType ]; then echo "COPY files/$dockerType $ScriptDir"  >> $dockerfile; fi
    if [ -f files/$dockerType/ca_certs ]; then echo "COPY /etc/pki/ca-trust/source/anchors/ca_certs"  >> $dockerfile; fi
  fi
...
join_json_files() {
  ...
  if [ ! -f ${jsonfiles}/dps-setup.json ]; then
    ...
    jq  --arg type "$DOCKER_TYPE" --arg cloudprovider "$CLOUD_PROVIDER" '.dockerType = $type | .cloudProvider = $cloudprovider' ${jsonfiles}/dps-setup-app.json > ${jsonfiles}/dps-setup-$DOCKER_TYPE.json
...
change_dockerType_json_keys() {

  local jsonfile=$1
  local dockertype=$2
  
  old_value=$(jq -r '.dockerType' ${jsonfiles}/dps-setup.json)
  jq --arg old_val "$old_value" --arg new_val "$dockertype" \
     '(.dockerType | select(. == $old_val)) = $new_val' "${jsonfiles}/dps-setup.json" > tmp.json && mv -f tmp.json "${jsonfiles}/dps-setup.json"
}
...
folders() {
    ...
    ServiceBackupDir=${RootBackupDir}/$dockerType
