...
container_exists "${DOCKERNAME}_${TYPE}" $type
if [[ "$run" == "podman" ]]; then
  type="podman"
  if [[ "$LIMITS" == "YES" ]]; then
...
help() {
  local docker_types
  docker_types=$(grep dockerType docs/containerTypes.csv | sed -n 1'p' | tr ',' '\n' | grep -v dockerType | awk '{print}' ORS='|')
